...
VOID tx_application_define(VOID *first_unused_memory)
{
  ...
  printf("tx_application_define: start\r\n");
  ...
  else
  {
    ...
    if (status != NX_SUCCESS)
    {
      ...
      printf("MX_NetXDuo_Init FAILED, status=%u\r\n", status);
      while(1)
      {
      }
  ...
  else
  {
    ...
    if (status != UX_SUCCESS)
    {
      ...
      printf("MX_USBX_Init FAILED, status=%u\r\n", status);
      while(1)
      {
      }
