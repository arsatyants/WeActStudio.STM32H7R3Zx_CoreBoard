    ...
    while (device_framework_length != 0)
    {
        ...
        if(descriptor_type == UX_INTERFACE_DESCRIPTOR_ITEM)
        {
...

            {
                ...
                if (dbg_counters[10] < 9)
                {
                    dbg_counters[11 + dbg_counters[10]] =
                        ((ULONG)interface_descriptor.bInterfaceNumber << 8) |
                        ((ULONG)interface_descriptor.bAlternateSetting << 4) | iad_flag;
                    dbg_counters[10]++;
                }
            }
