...
UX_SLAVE_CLASS_COMMAND      class_command;
...

    {
        ...
        ULONG itf3 = interface_ptr->ux_slave_interface_descriptor.bInterfaceNumber;
        if (itf3 < 2)
        {
            dbg_counters[25 + itf3 * 2] = itf3;
            dbg_counters[26 + itf3 * 2] = interface_ptr->ux_slave_interface_descriptor.bNumEndpoints;
        }
    }
    ...
    if (status == UX_SUCCESS)
    {
        ...
        {
            extern volatile ULONG dbg_counters[30];
            ULONG itf2 = interface_ptr->ux_slave_interface_descriptor.bInterfaceNumber;
            if (itf2 < 2)
            {
                dbg_counters[20 + itf2] = interface_ptr->ux_slave_interface_descriptor.bNumEndpoints;
                dbg_counters[22 + itf2] = (ULONG)(uintptr_t)interface_ptr;
            }
        }
