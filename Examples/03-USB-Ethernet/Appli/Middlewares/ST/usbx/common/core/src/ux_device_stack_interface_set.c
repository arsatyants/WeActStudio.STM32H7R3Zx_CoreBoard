...
ULONG                   max_transfer_length, n_trans;
...

    {
        ...
        ULONG itf = interface_ptr->ux_slave_interface_descriptor.bInterfaceNumber;
        if (itf < 2)
        {
            dbg_counters[14 + itf * 2] = interface_ptr->ux_slave_interface_descriptor.bNumEndpoints;
            dbg_counters[15 + itf * 2] = device_framework[4]; /* raw bNumEndpoints byte from device_framework */
        }
    }
