...
ULONG                           max_transfer_length, n_trans;
    ...
    printf("USBX alt_setting_set: itf=%lu alt=%lu dev_state=%u\r\n",
           interface_value, alternate_setting_value, device->ux_slave_device_state);

    /* SWD-readable debug counters, see main.c */
    extern volatile ULONG dbg_counters[30];
    dbg_counters[0]++;
    dbg_counters[1] = interface_value;
    dbg_counters[2] = alternate_setting_value;
    ...
    printf("alt_setting_set: found itf, current_alt=%u requested_alt=%lu\r\n",
           interface_ptr->ux_slave_interface_descriptor.bAlternateSetting, alternate_setting_value);
    ...
    {
        printf("alt_setting_set: NO-OP (already at requested alt)\r\n");
        return(UX_SUCCESS);
    }
